...
@torch.no_grad()
def show_prediction(model, dataset, device, idx=None, score_thresh=0.3, nms_thresh=0.5):
    model.eval()
    if idx is None:
    ...
    # boxes are the [0, 1] normalized predictions
    boxes = pred_bboxes[0].cpu() 
    scores = scores.cpu()
    labels = labels.cpu()
    ...
    if len(boxes) == 0:
        print("[INFO] No predictions above score threshold")
        return

    # === Apply NMS per class (on normalized [0, 1] boxes) ===
    ...
    for cls in unique_labels:
        ...
        # Run NMS on the [0, 1] coordinates
        keep_idx = nms(cls_boxes, cls_scores, nms_thresh)
        
        final_boxes.append(cls_boxes[keep_idx].numpy())
        final_scores.append(cls_scores[keep_idx].numpy())
        final_labels.append(labels[cls_mask][keep_idx].cpu().numpy())

    if len(final_boxes) == 0:
        print("[INFO] No predictions survived NMS")
        return
    ...
    # === Scale FINAL boxes to full-resolution (1936x1216) ===
    scale_x = FULLRES_SIZE[0]  # 1936
    scale_y = FULLRES_SIZE[1]  # 1216
    final_boxes = final_boxes * np.array([scale_x, scale_y, scale_x, scale_y])

    # === Load full-resolution image ===
    img_name = f"{idx:05d}.jpg"
    full_img_path = os.path.join(FULLRES_IMAGE_DIR, img_name)
    if not os.path.exists(full_img_path):
        print(f"[ERROR] Full-res image not found at {full_img_path}")
        return
    full_img = np.array(Image.open(full_img_path).convert("RGB"))
    ...
    for (x1, y1, x2, y2), sc, lab in zip(final_boxes, final_scores, final_labels):
    ...
    # === Draw GT boxes (scaled correctly) ===
    gt_mask = mask > 0
    gt_boxes = gt_bboxes[gt_mask].numpy()
    # Also scale GT boxes from [0, 1] to full resolution
    gt_boxes = gt_boxes * np.array([scale_x, scale_y, scale_x, scale_y]) 
    gt_labels_i = gt_labels[gt_mask].numpy()
    
    for (x1, y1, x2, y2), lab in zip(gt_boxes, gt_labels_i):
        rect = patches.Rectangle(
            (x1, y1),
            x2 - x1,
            y2 - y1,
...
def __main__():
    ...
    show_prediction(model, dataset, device, idx=42, score_thresh=0.7)
