...
from vod.frame import FrameDataLoader, FrameTransformMatrix # type: ignore
...
for frame_number in tqdm(frame_numbers):
    try:
        ...
        # Use raw label strings directly instead of FrameLabels
        for line in frame_data.raw_labels:
            ...
            # Extract the 2D bbox (columns 4–7)
            try:
                x_min = float(parts[4])
                y_min = float(parts[5])
                x_max = float(parts[6])
                ...
                bbox_2d = [x_min, y_min, x_max, y_max] if (x_max > x_min and y_max > y_min) else None

            # Extract 3D box data (columns 8–14)
            ...
            frame_ann["objects"].append({
                "class": cls,
                "bbox_3d": {"x": x, "y": y, "z": z, "h": h, "w": w, "l": l, "rotation": rotation},
                "bbox_2d": bbox_2d
            })
...
print(f"Preprocessing complete. Saved radar data, radar projections, and annotations.json to {output_root}")
